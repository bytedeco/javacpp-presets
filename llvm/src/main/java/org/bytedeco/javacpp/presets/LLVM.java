...
@Properties(target = "org.bytedeco.javacpp.LLVM", value = {@Platform(value = {"windows-x86", "windows-x86_64", "linux-x86", "macosx"}, define = {"__STDC_LIMIT_MACROS", "__STDC_CONSTANT_MACROS"},
    ...
    compiler = "cpp11", link = {"LLVM-4.0", "LTO"}), @Platform(value = "macosx", link = {"LLVM", "LTO"}), 
				@Platform(value = "windows", link = {"LTO", "LLVM"}) })
public class LLVM implements InfoMapper {
    public void map(InfoMap infoMap) {
        infoMap.put(new Info("LLVMOpaqueContext").pointerTypes("LLVMContextRef"))
               ...
               .put(new Info("LLVMDumpType").skip())   // TODO: Should only be done for the windows version
