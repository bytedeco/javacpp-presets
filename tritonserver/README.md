 ...
 * Triton Inference Server 2.32.0  https://github.com/triton-inference-server/server
...
```bash
 $ wget https://github.com/triton-inference-server/server/archive/refs/tags/v2.32.0.tar.gz
 $ tar zxvf v2.32.0.tar.gz
 $ cd server-2.32.0/docs/examples/model_repository
 $ mkdir models
...
```bash
 $ docker run -it --gpus=all -v $(pwd):/workspace nvcr.io/nvidia/tritonserver:23.03-py3 bash
 $ apt update
