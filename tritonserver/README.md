 ...
 * Triton Inference Server 2.70.0  https://github.com/triton-inference-server/server
...
```bash
 $ wget https://github.com/triton-inference-server/server/archive/refs/tags/v2.70.0.tar.gz
 $ tar zxvf v2.70.0.tar.gz
 $ cd server-2.70.0/docs/examples/model_repository
 $ mkdir models
...
```bash
 $ docker run -it --gpus=all -v $(pwd):/workspace nvcr.io/nvidia/tritonserver:26.06-py3 bash
 $ apt update
