 ...
 * Triton Inference Server 2.68.0  https://github.com/triton-inference-server/server
...
```bash
 $ wget https://github.com/triton-inference-server/server/archive/refs/tags/v2.68.0.tar.gz
 $ tar zxvf v2.68.0.tar.gz
 $ cd server-2.68.0/docs/examples/model_repository
 $ mkdir models
...
```bash
 $ docker run -it --gpus=all -v $(pwd):/workspace nvcr.io/nvidia/tritonserver:26.04-py3 bash
 $ apt update
