 ...
 * Triton Inference Server 2.61.0  https://github.com/triton-inference-server/server
...
```bash
 $ wget https://github.com/triton-inference-server/server/archive/refs/tags/v2.61.0.tar.gz
 $ tar zxvf v2.61.0.tar.gz
 $ cd server-2.61.0/docs/examples/model_repository
 $ mkdir models
...
```bash
 $ docker run -it --gpus=all -v $(pwd):/workspace nvcr.io/nvidia/tritonserver:25.09-py3 bash
 $ apt update
