...
patch -Np1 --binary -d ARToolKitPlus-$ARTOOLKITPLUS_VERSION < ../ARToolKitPlus-$ARTOOLKITPLUS_VERSION.patch
mkdir ARToolKitPlus-$ARTOOLKITPLUS_VERSION/build_$PLATFORM
cd ARToolKitPlus-$ARTOOLKITPLUS_VERSION/build_$PLATFORM

case $PLATFORM in
    android-arm)
        cmake -DCMAKE_TOOLCHAIN_FILE=android-arm.cmake -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX="$ANDROID_NDK/../" ..
        make -j4
        make install
        ;;
    linux-x86)
