...
public class RecordBatchFileReader extends Pointer {
  ...
  public static native @ByVal RecordBatchFileReaderFuture OpenAsync(
        @SharedPtr @Cast({"", "std::shared_ptr<arrow::io::RandomAccessFile>"}) RandomAccessFile file,
        @Const @ByRef(nullValue = "arrow::ipc::IpcReadOptions::Defaults()") IpcReadOptions options);
  public static native @ByVal RecordBatchFileReaderFuture OpenAsync(
        @SharedPtr @Cast({"", "std::shared_ptr<arrow::io::RandomAccessFile>"}) RandomAccessFile file);
  ...
  /** \brief Open a file asynchronously (owns the file). */
  public static native @ByVal RecordBatchFileReaderFuture OpenAsync(
        @SharedPtr @Cast({"", "std::shared_ptr<arrow::io::RandomAccessFile>"}) RandomAccessFile file, @Cast("int64_t") long footer_offset,
        @Const @ByRef(nullValue = "arrow::ipc::IpcReadOptions::Defaults()") IpcReadOptions options);
  public static native @ByVal RecordBatchFileReaderFuture OpenAsync(
        @SharedPtr @Cast({"", "std::shared_ptr<arrow::io::RandomAccessFile>"}) RandomAccessFile file, @Cast("int64_t") long footer_offset);
   ...
   * 
   *  @param coalesce [in] If true, enable I/O coalescing.
   *  @param io_context [in] The IOContext to use (controls which thread pool
   *      is used for I/O).
   *  @param cache_options [in] Options for coalescing (if enabled).
   *  @param executor [in] Optionally, an executor to use for decoding record
