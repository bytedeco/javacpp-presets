...
public class DNAConvTest {
    ...
    public void testAttentionNormalization() {
        ...
        Tensor Q_flat = dnaConvNoBias.getLinQ().forward(x_flat);
        Tensor K_flat = dnaConvNoBias.getLinK().forward(x_flat);
    ...
    public void testInvalidInput() {
        ...
        // 测试2：heads/groups 为0
        assertThrows(IllegalArgumentException.class, () -> new DNAConv(4, 0, 2, true));

        // 测试3：x 维度错误（4维）— 2维会被自动升为 [N,1,C]
        Tensor x4d = torch.randn(new long[]{5, 3, 4, 2});
        assertThrows(IllegalArgumentException.class, () -> ((DNAConv)dnaConv).forward(x4d, edgeIndex));
...
//        torch.cuda.empty_cache();
    ...
    public static void main(String[] args) {
        Result result = JUnitCore.runClasses(DNAConvTest.class);
        ...
        if (!result.wasSuccessful()) {
            throw new RuntimeException("DNAConvTest had " + result.getFailureCount() + " failures");
        }
        System.out.println("✅ DNAConvTest all tests passed");
    }
