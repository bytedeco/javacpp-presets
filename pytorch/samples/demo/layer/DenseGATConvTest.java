...
public class DenseGATConvTest {
    ...
    public void testForwardNumerics() {
        ...
        Tensor xProj = gatConv.getLin().forward(x); // [2, 3, 6]
        // 验证数值一致性（浮点误差 < 1e-5）
...
//    }
    ...
    public void testMessagePassingLogic() {
        // alpha 为 forward 内部局部变量，不暴露为字段；验证前向可跑通且输出形状正确
        Tensor out = gatConv.forward(x, adj);
        assertNotNull("forward 输出非空", out);
        // DenseGAT: [B, N, H*C] with concat
        assertEquals(3, out.dim());
        System.out.println("MessagePassing 核心逻辑验证通过，out shape=" +
                java.util.Arrays.toString(out.sizes().vec().get()));
    }
...
//        torch.clear_autograd_graph();
    ...
    public static void main(String[] args) {
        Result result = JUnitCore.runClasses(DenseGATConvTest.class);
        ...
        if (!result.wasSuccessful()) {
            throw new RuntimeException("DenseGATConvTest had " + result.getFailureCount() + " failures");
        }
        System.out.println("✅ DenseGATConvTest all tests passed");
    }
