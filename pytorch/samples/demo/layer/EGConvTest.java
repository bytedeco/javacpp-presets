...
//        }
    ...
    public void testNumericalAccuracy2() {
        ...
        try (NoGradGuard g = new NoGradGuard()) { egConv.basesWeights.copy_(basesWeights); }
        // 系数线性层权重设为0，偏置设为1（确保系数为1）
        try (NoGradGuard g = new NoGradGuard()) { egConv.linCoeffs.weight().zero_(); }
        try (NoGradGuard g = new NoGradGuard()) { egConv.linCoeffs.bias().fill_(torch.tensor(1.0f).item()); }
...
//        }
    ...
    public void testMultiAggregator() {
        ...
        try (NoGradGuard g = new NoGradGuard()) { egConv.basesWeights.copy_(eyeTensor); }
        try (NoGradGuard g = new NoGradGuard()) { egConv.linCoeffs.weight().zero_(); }
        try (NoGradGuard g = new NoGradGuard()) { egConv.linCoeffs.bias().fill_(torch.tensor(1.0f).item()); }
    ...
    public static void main(String[] args) {
        Result result = JUnitCore.runClasses(EGConvTest.class);
        ...
        if (!result.wasSuccessful()) {
            throw new RuntimeException("EGConvTest had " + result.getFailureCount() + " failures");
        }
        System.out.println("✅ EGConvTest all tests passed");
    }
