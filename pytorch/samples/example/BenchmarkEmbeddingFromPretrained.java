...
public class BenchmarkEmbeddingFromPretrained {
    ...
    private static void runEmbeddingBagBench() {
        ...
        Tensor indices = arange(new Scalar(0), new Scalar(8192))
                .remainder(new Scalar(2048));
        Tensor offsets = arange(new Scalar(0), new Scalar(128))
                .mul(new Scalar(64));

        long t0 = System.nanoTime();
        EmbeddingBagImpl embeddingBag = EmbeddingBagImpl.from_pretrained(weights, new EmbeddingBagFromPretrainedOptions());
        float init = embeddingBag.forward(indices, offsets, (Tensor)null).sum().item_float();
        float last = init;
        for (int i = 0; i < steps; i++) {
            last = embeddingBag.forward(indices, offsets, (Tensor)null).sum().item_float();
        }
        float fin = embeddingBag.forward(indices, offsets, (Tensor)null).sum().item_float();
