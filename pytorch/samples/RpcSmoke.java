...
public class RpcSmoke {
    ...
    public static void main(String[] args) {
        System.out.println("[RpcSmoke] starting ...");
        ...
        System.out.println("[RpcSmoke] OK");
    }
    ...
    private static void checkLoaded(String fqn, String label) {
        try {
            ...
            System.out.println("[RpcSmoke]  bound  " + fqn + "  (" + label + ")");
    ...
    private static void benchEnumConstants() {
        System.out.println("[RpcSmoke] benchEnumConstants");
        ...
        try {
            ...
            System.out.println("[RpcSmoke]  reflected " + ok + " of " + names.length + " enum constants from global");
        } catch (Throwable t) {
            System.out.println("[RpcSmoke]  (skipping: " + t.getClass().getSimpleName() + ": " + t.getMessage() + ")");
        }
        long t1 = nowNanos();
        System.out.printf("[RpcSmoke]  reflectively read 8 enum constants : %.2f ms  (%.0f ns/op)%n",
                ms(t1 - t0), (double) (t1 - t0) / 8);
    }

    private static void benchRpcErrorTypeConstants() {
        System.out.println("[RpcSmoke] benchRpcErrorTypeConstants");
        try {
            Class<?> wp = Class.forName("org.bytedeco.pytorch.rpc.WorkerInfo");
            System.out.println("[RpcSmoke]  WorkerInfo fields: "
                    + java.util.Arrays.toString(wp.getDeclaredFields()));
        } catch (Throwable t) {
            System.out.println("[RpcSmoke]  (skipping: " + t + ")");
        }
    }
    ...
    private static void benchValueTypeConstruction() {
        System.out.println("[RpcSmoke] benchValueTypeConstruction");
        ...
        System.out.println("[RpcSmoke]  TensorPipeRpcBackendOptions.numWorkerThreads bound="
                + hasMethod(TensorPipeRpcBackendOptions.class, "numWorkerThreads"));
        System.out.println("[RpcSmoke]  RpcRetryOptions.maxRetries bound="
                + hasMethod(RpcRetryOptions.class, "maxRetries"));
        System.out.println("[RpcSmoke]  WorkerInfo.name_/id_ bound="
                + (hasMethod(WorkerInfo.class, "name_") && hasMethod(WorkerInfo.class, "id_")));
    }

    private static void benchRpcRetryOptionsMutation() {
        System.out.println("[RpcSmoke] benchRpcRetryOptionsMutation");
        if (!hasMethod(RpcRetryOptions.class, "maxRetries")) {
            System.out.println("[RpcSmoke]  skip mutation — maxRetries not bound");
            return;
        }
        System.out.println("[RpcSmoke]  skip mutation — default RpcRetryOptions ctor not bound "
    ...
    private static void probeDefaultConstruction(Class<?> cls) {
        try {
            ...
            System.out.println("[RpcSmoke]  " + cls.getSimpleName() + " default ctor: OK");
        } catch (NoSuchMethodException e) {
            System.out.println("[RpcSmoke]  " + cls.getSimpleName()
                    + " default ctor: NOT BOUND (Pointer cast only) — skip alloc bench");
        } catch (Throwable t) {
            System.out.println("[RpcSmoke]  " + cls.getSimpleName() + " default ctor failed: " + t);
        }
    }
    ...
    private static void probeConstruction(Class<?> cls, Class<?>[] types, Object... args) {
        try {
            ...
            if (o instanceof Pointer p) p.close();
            System.out.println("[RpcSmoke]  " + cls.getSimpleName() + " value ctor: OK");
        } catch (NoSuchMethodException e) {
            System.out.println("[RpcSmoke]  " + cls.getSimpleName()
                    + " value ctor " + java.util.Arrays.toString(types)
                    + ": NOT BOUND — skip alloc bench");
        } catch (Throwable t) {
            System.out.println("[RpcSmoke]  " + cls.getSimpleName() + " value ctor failed: " + t);
        }
    }
}
