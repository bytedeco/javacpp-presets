...
public final class ClipStyleEmbeddingModel implements EmbeddingModel {
    ...
    static Modality detect(Object input) {
        if (input instanceof org.bytedeco.pytorch.data.dataframe.dtype.ImageData) return Modality.IMAGE;
        if (input instanceof org.bytedeco.pytorch.data.dataframe.dtype.AudioData) return Modality.AUDIO;
        if (input instanceof org.bytedeco.pytorch.data.dataframe.dtype.VideoData) return Modality.VIDEO;
        if (input instanceof org.bytedeco.pytorch.data.dataframe.dtype.TensorData) return Modality.TENSOR;
