...
public class AudioData extends AbstractDataValue implements MediaData {
        ...
        if (audioFormat != AudioFormat.WAV) {
            try {
                Class<?> bridge = Class.forName(
                        "org.bytedeco.pytorch.data.dataframe.media.MediaBridge");
    ...
    private float estimateTempo(float[] onsetStrength) {
        if (onsetStrength.length < 2) return 120f;

        int maxLag = Math.min(onsetStrength.length / 2, sampleRate / 2);
        float maxCorr = 0;
        int bestLag = sampleRate / 2;

        for (int lag = sampleRate / 10; lag < maxLag; lag++) {
            float correlation = 0;
        ...
        float lagInSeconds = (float) bestLag * DEFAULT_HOP_LENGTH / sampleRate;
        return 60f / lagInSeconds;
    }
