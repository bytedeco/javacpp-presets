...
public class RandomForestClassifier extends BaseClassifier {
    ...
    public BaseClassifier fit(double[][] X, double[] y) {
        ...
        int mf = maxFeatures == null ? (int) Math.sqrt(d) : Math.min(maxFeatures, d);
        ...
        for (int t = 0; t < nEstimators; t++) {
            ...
            // Random feature subset (modify bX columns)
            ...
            tree.fit(subX, bY);
            trees.add(tree);
            // store feature indices in tree? We need to remember them for predict
