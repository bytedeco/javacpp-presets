...
public final class TorchAudioEmbeddingModel implements EmbeddingModel {
    ...
    private float[] embedAudio(AudioData audio) {
        ...
        if (use.getChannels() > 1) {
            try {
                use = org.bytedeco.pytorch.data.dataframe.media.MediaBridge.toMono(use);
            } catch (Throwable ignored) {}
        }
        // resample
        if (use.getSampleRate() > 0 && use.getSampleRate() != targetSampleRate) {
            try {
                use = org.bytedeco.pytorch.data.dataframe.media.MediaBridge.resample(use, targetSampleRate);
            } catch (Throwable ignored) {}
        }
    ...
    private static AudioData coerceAudio(Object input) {
        ...
        if (input instanceof String path) {
            try {
                return org.bytedeco.pytorch.data.dataframe.media.MediaBridge.loadAudio(path, 16000, true);
            } catch (Exception e) {
                return null;
            }
        }
