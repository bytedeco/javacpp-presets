...
/** SPI for local / remote inference backends. */
public interface InferenceEngine extends AutoCloseable {

    String name();

    void load(LoadRequest request) throws Exception;

    boolean isLoaded();

    Optional<String> loadedModelId();

    ChatCompletionResponse chatCompletions(ChatCompletionRequest request) throws Exception;

    default Map<String, Object> stats() {
        return Map.of("engine", name(), "loaded", isLoaded());
    }
