...
/**
 * Compatibility alias for older demos that referenced {@code CoWBlockManagerV8}.
 * Delegates to {@link CoWBlockManager} (numHeads=1).
 */
public class CoWBlockManagerV8 extends CoWBlockManager {
    public CoWBlockManagerV8(int totalBlocks, int numLayers, int blockSize, int headDim, int dtypeValue) {
        super(totalBlocks, numLayers, blockSize, 1, headDim, null, resolve(dtypeValue));
    }

    public CoWBlockManagerV8(int totalBlocks, int numLayers, int blockSize, int numHeads, int headDim,
