...
/** Mixtral / SmallThinker sparse MoE injection plan. */
public final class MixtralFamilyInject {

    private MixtralFamilyInject() {}

    public static LayerInjectPlan plan(KtModelFamily family) {
        KtModelFamily f = family != null ? family : KtModelFamily.MIXTRAL;
        return LayerInjectPlan.builder(f)
                ...
                .moeFfnGlobs("*.block_sparse_moe.*", "*.experts.*")
                .recommendedQuant(KtQuantConfig.int8AmxLike())
                .note("family", f.name())
                .note("moe", "Mixtral block_sparse_moe")
                .build();
    }
}
