...
public class LogisticRegression extends BaseClassifier {
    ...
    /** Mean absolute weights across classes — for SelectFromModel. */
    public double[] getCoef() {
        if (weights == null) return new double[0];
        int d = weights[0].length;
        double[] out = new double[d];
        for (double[] w : weights) {
            for (int j = 0; j < d; j++) out[j] += Math.abs(w[j]);
        }
        for (int j = 0; j < d; j++) out[j] /= weights.length;
        return out;
    }
