...
public class CoWBlockManagerV9 extends CoWBlockManagerV2 {
    ...
    // ======================== 修正并完善：allocateBlocks 方法 ========================
    /**
     * 为会话分配指定数量的物理缓存块（适配PageAttention接口）
     * 整合原有allocateWithRetry的重试、驱逐、水位控制逻辑
     * @param numBlocks 需要分配的块数
     * @param blockSize 每个块的token数（需与管理器配置一致）
     */
    public void allocateBlocks(int numBlocks, int blockSize) {
        // 校验块大小一致性
        if (blockSize != this.actualBlockSize) {
            throw new IllegalArgumentException("块大小不匹配：管理器配置=" + this.actualBlockSize + "，请求=" + blockSize);
        }
        ...
        try {
            ...
            for (int i = 0; i < numBlocks; i++) {
                // 复用原有allocateWithRetry逻辑处理块分配、重试、驱逐
                allocateWithRetry("temp_session", null); // 若有具体sessionId可替换第一个参数
            }
        } finally {
            super.getGlobalLock().unlock();
        }

        // 分配完成后检查水位，触发剪枝（保持原有逻辑）
        checkAndPrune();
    }

    // ======================== 完善：releaseBlocks 方法 ========================
    /**
     * 释放指定会话绑定的所有物理块（适配PageAttention接口）
     * 对齐原有releaseSession的引用计数逻辑，避免重复释放
    ...
    // ======================== 优化：getPhysicalBlockIds 方法 ========================
    /**
     * 获取指定会话已分配的物理块ID数组（适配PageAttention接口）
     * 修正返回类型为long[]，并处理空值、异常情况
     * @param sessionId 会话ID
     * @return 物理块ID数组，无分配块时返回空数组
     */
    public long[] getPhysicalBlockIds(String sessionId) {
        ...
        try {
            ...
            // 转换为long数组（适配PageAttention的Tensor构建）
