...
public class DenseGCNConvTest {
    ...
    public void testForwardNumerics() {
        ...
        Tensor out = gcnConv.forward(x, adj, new TensorOptional());
        // 2. 线性投影值（X @ W）
        Tensor xW = gcnConv.lin.forward(x);
...
//
//        // 5. 构造掩码：[2, 3]，屏蔽最后一个节点（值为 0）
//        // ========== 修复核心：适配 JavaCPP 的 put_ 方法 ==========
//        mask = torch.ones(new long[]{2, 3});
//        mask.select(0, 0).select(0, 2).copy_(torch.tensor(0f));
//        // 第 1 批次的第 2 个节点
//        mask.select(0, 1).select(0, 2).copy_(torch.tensor(0f));
//        // 方式1：单索引+单值赋值（推荐，适配 JavaCPP 行为）
//        // 第 0 批次的第 2 个节点设为 0
////        mask.put_(torch.tensor(new long[]{0, 2}).to(torch.ScalarType.Long), torch.tensor(0f));
////        // 第 1 批次的第 2 个节点设为 0
////        mask.put_(torch.tensor(new long[]{1, 2}).to( torch.ScalarType.Long), torch.tensor(0f));
//
//        // 方式2（批量赋值）：索引张量和源张量元素数一致（备选方案）
//        // Tensor indices = torch.tensor(new long[][]{{0,2}, {1,2}}, torch.ScalarType.Long);
//        // Tensor values = torch.tensor(new float[]{0f, 0f});
//        // mask.put_(indices, values);
//    }
//
//    @Test
//    public void testForwardShape() {
...
//        assertArrayEquals(out.sizes().vec().get(), improvedOut.sizes().vec().get());
//
//        System.out.println("输出形状验证通过：" + out.sizes());
//    }
//
//    @Test
//    public void testForwardNumerics() {
