...
public class GMMConvTest {
    ...
    public void testForwardShape() {
        ...
        GMMConv gmmConv = new GMMConv(inChannels, outChannels, dim, kernelSize,
                true, true, device, torch.ScalarType.Float);
    ...
    public void testNumericalAccuracy() {
        ...
        Device device = new Device(torch.kCPU());
        GMMConv gmmConv = new GMMConv(inChannels, outChannels, dim, kernelSize,
                false, false, device, torch.ScalarType.Float);
...
//        }
    ...
    public void testEdgeCases2() {
        ...
        // ========== 补充验证：根节点变换结果匹配 ==========
        Tensor rootOut = gmmConv.linRoot.forward(x);
...
//        }
        ...
        // 验证单节点输出等于根节点变换结果
        Tensor rootSingle = gmmConv.linRoot.forward(xSingle);
        assertEquals("单节点输出维度应为1×1", 1, outputSingle.sizes().vec().get()[1]);
