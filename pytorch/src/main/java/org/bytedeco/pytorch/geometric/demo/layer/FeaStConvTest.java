...
public class FeaStConvTest {
    ...
    public void testNumericalAccuracy() {
        ...
        try (NoGradGuard guard = new NoGradGuard()) {
            ...
            feastConv.linWeights.weight().copy_(linWeights);
            feastConv.linWeights.bias().copy_(torch.zeros(new long[]{1}));

            // 固定linSrc/linDst权重：[1, 2] → [[1, 0]]（输出=输入第1维）
            Tensor linSrcWeight = torch.tensor(new float[]{1.0f, 0.0f}).view(1,2);
            feastConv.linSrc.weight().copy_(linSrcWeight);
            feastConv.linSrc.bias().copy_(torch.zeros(new long[]{1}));

            Tensor linDstWeight = torch.tensor(new float[]{1.0f, 0.0f}).view(1,2);
            feastConv.linDst.weight().copy_(linDstWeight);
            feastConv.linDst.bias().copy_(torch.zeros(new long[]{1}));
        }
...
//        }
    ...
    public void testEdgeCases() {
        ...
        // 空边时：输出=全0 + 偏置
        Tensor expectedEmpty = torch.zeros(new long[]{3, outChannels}, floatOpts).add(feastConv.bias.data());
