...
public class Qwen3InferenceBenchmarkv8 {
    // CoWBlockManagerV9：优化的Copy-on-Write块管理器，解决原有缓存管理问题
    public static class CoWBlockManagerV9 {
        private static final int BLOCK_SIZE = 256; // 块大小，适配PageAttention
        private final ConcurrentHashMap<Long, Tensor> blockCache; // 块缓存
        private final ConcurrentHashMap<String, Long> sessionBlockMap; // 会话-块映射
        private long nextBlockId = 0;

        public CoWBlockManagerV9() {
            this.blockCache = new ConcurrentHashMap<>();
            this.sessionBlockMap = new ConcurrentHashMap<>();
        }
         ...
         * @return 缓存块Tensor
         */
        public Tensor getBlock(String sessionId, int seqLen) {
            long blockId = getBlockId(sessionId);
            int blockIndex = seqLen / BLOCK_SIZE;
            ...
            if (!blockCache.containsKey(blockId)) {
                ...
                Tensor newBlock = torch.zeros(new long[]{1, BLOCK_SIZE, 4096}, options);
                blockCache.put(blockId, newBlock);
            }
        ...
        /**
         * 写入数据到缓存块（Copy-on-Write）
         * @param sessionId 会话ID
         * @param seqLen 当前序列长度
         * @param data 要写入的数据
         */
        public void writeBlock(String sessionId, int seqLen, Tensor data) {
            long blockId = getBlockId(sessionId);
            int offset = seqLen % BLOCK_SIZE;

            // Copy-on-Write：写操作时复制块，避免并发冲突
            Tensor originalBlock = blockCache.get(blockId);
            if (originalBlock != null) {
                Tensor newBlock = originalBlock.clone();
                // 将数据写入指定位置
                newBlock.narrow(1, offset, 1).copy_(data);
                blockCache.put(blockId, newBlock);
         ...
         * @param sessionId 会话ID
         */
        public void cleanSessionBlocks(String sessionId) {
            Long blockId = sessionBlockMap.remove(sessionId);
            if (blockId != null) {
                Tensor block = blockCache.remove(blockId);
                if (block != null) {
                    block.close();
                }
            }
        ...
        public void cleanAllBlocks() {
            // 释放所有块资源
            for (Tensor block : blockCache.values()) {
                block.close();
            }
            blockCache.clear();
            sessionBlockMap.clear();
        }

        /**
         * 获取会话对应的块ID
         */
        private long getBlockId(String sessionId) {
            return sessionBlockMap.computeIfAbsent(sessionId, k -> nextBlockId++);
        }
    ...
    public static class Qwen3JavaInferenceV3 {
        ...
        public Qwen3JavaInferenceV3(String modelPath) {
            ...
            // 初始化优化的缓存管理器
            this.blockManager = new CoWBlockManagerV9();
            this.kvCacheMap = new ConcurrentHashMap<>();

            // 禁用梯度计算，提升推理性能
            torch.requires_grad(false);
//            torch.no_grad(); // 启用梯度禁用
        }

        /**
         * 集成CoWBlockManagerV9和PageAttention的generate方法
        ...
        public Tensor generate(Tensor inputIds, int generateLen, String sessionId) {
            ...
            try {
                ...
                // 逐Token生成（自回归生成）- 使用CoWBlockManagerV9优化缓存
                for (int step = 0; step < generateLen; step++) {
                    // 边界检查：不超过最大序列长度
                    if (cache.seqLen + inputSeqLen >= MAX_SEQ_LEN) {
                        throw new RuntimeException("序列长度超过最大值：" + MAX_SEQ_LEN);
                    }

                    // 使用CoWBlockManagerV9获取缓存块
                    Tensor cacheBlock = blockManager.getBlock(sessionId, cache.seqLen);
                    ...
                    // 5. 使用CoWBlockManagerV9写入缓存
                    blockManager.writeBlock(sessionId, cache.seqLen, nextToken);
                ...
                blockManager.cleanSessionBlocks(sessionId);
        ...
        private Tensor forwardWithKVCache(Tensor inputIds, CacheEntry cache, boolean isInitial) {
            ...
            // 直接返回logits（使用CoWBlockManagerV9管理缓存）
            return output.toTensor();
        }

        /**
         * PageAttention实现 - 基于CoWBlockManagerV9的外部分页优化
         * @param cache 原始KV缓存
         * @return 分页优化后的缓存
         */
        private CacheEntry optimizeKVCacheWithPageAttention(CacheEntry cache) {
            ...
            // 清理过期页面（超过3页的历史缓存）
            if (cache.lastPageIdx > 3) {
                cache.seqLen = (int) (cache.lastPageIdx * PAGE_SIZE);
            }
            return cache;
        }
        ...
        /**
         * 清理指定会话的KV Cache（使用CoWBlockManagerV9）
         */
        ...
        public void clearAllKVCache() {
            ...
            kvCacheMap.clear();
            // 清理所有块缓存
            blockManager.cleanAllBlocks();
        }

        // 释放资源
        public void close() {
            clearAllKVCache();
            blockManager.close(); // 关闭块管理器
            if (model != null) model.close();
            if (device != null) device.close();
        }

        // KV缓存条目（增强版，支持PageAttention和CoWBlockManagerV9）
        private static class CacheEntry {
            Tensor kCache;       // Key Cache（由CoWBlockManagerV9管理）
            Tensor vCache;       // Value Cache（由CoWBlockManagerV9管理）
            int seqLen = 0;      // 当前序列长度
    ...
    public static void main(String[] args) {
        ...
        // 执行压测
        System.out.println("===== 开始压测（集成CoWBlockManagerV9 + PageAttention） =====");
        System.out.printf("压测配置：轮数=%d, 每轮生成Token数=%d, PageSize=%d\n",
                testRound, generateLen, Qwen3JavaInferenceV3.PAGE_SIZE);

        // 预热轮次（消除首次加载影响）
        System.out.println("\n=== 预热轮次 ===");
        try {
            Tensor warmupOutput = inference.generate(inputIds, 10, "warmup_session");
            warmupOutput.close();
            inference.clearKVCache("warmup_session");
        ...
        for (int round = 0; round < testRound; round++) {
            ...
            try {
                Tensor output = inference.generate(inputIds, generateLen, sessionId + "_" + round);
                ...
                output.close();
                // 每轮结束清理缓存（模拟不同会话）
                inference.clearKVCache(sessionId + "_" + round);
     ...
     * Top-K采样（可选，替代贪心采样）
     */
    private static Tensor topKSample(Tensor logits, int k) {
