...
/** Context to save information during {@code forward} that can be accessed in {@code backward}
 *  in custom autograd operations (see {@code torch::autograd::Function} for details). */
...
public class AutogradContext extends Pointer {
  ...
  public native void save_for_backward(@Cast({"", "std::vector<at::Tensor>"}) @StdMove TensorVector to_save);
   ...
   *  should be inputs. */
  public native void mark_dirty(@Cast({"", "std::vector<at::Tensor>"}) @StdMove TensorVector inputs);
  /** Marks outputs in the list as not requiring gradients. This should be called
   *  at most once from inside of {@code forward} and all arguments should be outputs. */
  public native void mark_non_differentiable(@Cast({"", "std::vector<at::Tensor>"}) @StdMove TensorVector outputs);
   ...
   *  {@code save_for_backward()}. Before returning them to the user, a check is made to
   *  ensure that they were not modified by any in-place operations. */
  public native @Cast({"", "std::vector<at::Tensor>"}) @StdMove TensorVector get_saved_variables();
