...
public class DropoutDescriptor extends Pointer {
  ...
  public native void initialize_rng(@ByVal @Cast("cudnnHandle_t*") Pointer handle, float dropout, long seed, @Const @ByRef TensorOptions options);

  // Restore a dropout descriptor given a dropout probability and existing RNG state.
  public native void set(@ByVal @Cast("cudnnHandle_t*") Pointer handle, float dropout, @ByVal Tensor state_);

  // Restore a dropout descriptor corresponding to no dropout
  public native void set_no_dropout(@ByVal @Cast("cudnnHandle_t*") Pointer handle);
}
