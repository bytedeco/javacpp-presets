// Targeted by JavaCPP version 1.5.14-SNAPSHOT: DO NOT EDIT THIS FILE

package org.bytedeco.pytorch.nn;

import org.bytedeco.pytorch.jit.*;

import org.bytedeco.pytorch.data.*;

import org.bytedeco.pytorch.*;

import org.bytedeco.pytorch.Allocator;
import org.bytedeco.pytorch.jit.Function;
import org.bytedeco.javacpp.annotation.Cast;
import java.nio.*;
import org.bytedeco.javacpp.*;
import org.bytedeco.javacpp.annotation.*;

import static org.bytedeco.javacpp.presets.javacpp.*;
import static org.bytedeco.openblas.global.openblas_nolapack.*;
import static org.bytedeco.openblas.global.openblas.*;
import org.bytedeco.javacpp.chrono.*;
import static org.bytedeco.javacpp.global.chrono.*;

import static org.bytedeco.pytorch.global.torch.*;
 // namespace functional

// ============================================================================

/** Options for the {@code GELU} module.
 * 
 *  Example:
 *  <pre>{@code
 *  GELU model(GELUOptions().approximate("none"));
 *  }</pre> */
@Namespace("torch::nn") @Properties(inherit = org.bytedeco.pytorch.presets.torch.class)
public class GELUOptions extends Pointer {
    static { Loader.load(); }
    /** Default native constructor. */
    public GELUOptions() { super((Pointer)null); allocate(); }
    /** Native array allocator. Access with {@link Pointer#position(long)}. */
    public GELUOptions(long size) { super((Pointer)null); allocateArray(size); }
    /** Pointer cast constructor. Invokes {@link Pointer#Pointer(Pointer)}. */
    public GELUOptions(Pointer p) { super(p); }
    private native void allocate();
    private native void allocateArray(long size);
    @Override public GELUOptions position(long position) {
        return (GELUOptions)super.position(position);
    }
    @Override public GELUOptions getPointer(long i) {
        return new GELUOptions((Pointer)this).offsetAddress(i);
    }

  public native @StdString @ByRef @NoException(true) BytePointer approximate();
  public native @ByRef @NoException(true) GELUOptions approximate(@StdString BytePointer setter);
  public native @ByRef @NoException(true) GELUOptions approximate(@StdString String setter);
}
