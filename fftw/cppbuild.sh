...
case $PLATFORM in
    ...
    linux-armhf)
        ...
        ;;
    linux-arm64)
        ./configure --prefix=$INSTALL_PATH --disable-fortran --enable-shared --enable-threads --with-combined-threads CC="gcc"
        make -j $MAKEJ V=0
        make install-strip
        ./configure --prefix=$INSTALL_PATH --disable-fortran --enable-shared --enable-threads --with-combined-threads CC="gcc" --enable-float
        make -j $MAKEJ V=0
        make install-strip
        ;;
    linux-ppc64le)
