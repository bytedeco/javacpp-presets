...
@Name("std::vector<std::string>") @Properties(inherit = org.bytedeco.depthai.presets.depthai.class)
public class StringVector extends Pointer {
    ...
    public StringVector(ByteBuffer value) { this(1); put(0, value); }
    public StringVector(ByteBuffer ... array) { this(array.length); put(array); }
    ...
    @ValueSetter @Index(function = "at") public native StringVector put(@Cast("size_t") long i, @StdString ByteBuffer value);
    ...
    public StringVector push_back(ByteBuffer value) {
        long size = size();
        resize(size + 1);
        return put(size, value);
    }
    public StringVector put(ByteBuffer value) {
        if (size() != 1) { resize(1); }
        return put(0, value);
    }
    public StringVector put(ByteBuffer ... array) {
        if (size() != array.length) { resize(array.length); }
        for (int i = 0; i < array.length; i++) {
            put(i, array[i]);
        }
