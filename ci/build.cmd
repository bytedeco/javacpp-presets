...
IF "%APPVEYOR_PULL_REQUEST_NUMBER%"=="" (
   ...
   call mvn clean deploy -B -U -Dmaven.test.skip=true -Dmaven.javadoc.skip=true -Djavacpp.platform=windows-%MSYS2_ARCH% -Djavacpp.platform.extension=%EXT% -Djavacpp.copyResources --settings .\ci\settings.xml -pl .,%PROJ%
   IF ERRORLEVEL 1 (
   ...
   FOR %%a in ("%PROJ:,=" "%") do (
    ...
    cd %%a
    call mvn clean -B -U -f platform -Djavacpp.platform=windows-%MSYS2_ARCH% -Djavacpp.platform.extension=%EXT% --settings ..\ci\settings.xml deploy
    IF ERRORLEVEL 1 (
      echo Quitting with error  
      exit /b 1
    )
   ...
   echo Install %PROJ%
   call mvn clean install -B -U -Dmaven.test.skip=true -Dmaven.javadoc.skip=true -Djavacpp.platform=windows-%MSYS2_ARCH% -Djavacpp.platform.extension=%EXT% -Djavacpp.copyResources -pl .,%PROJ%
   IF ERRORLEVEL 1 (
      echo Quitting with error  
      exit /b 1 
   )
