...
IF /I "%APPVEYOR_REPO_BRANCH%"=="master" (
   ...
   call mvn deploy -Djavacpp.copyResources -Dmaven.test.skip=true -Dmaven.javadoc.skip=true -Djavacpp.platform=windows-%MSYS2_ARCH% --settings .\ci\settings.xml -pl %PROJ%
) ELSE (
   echo Install %PROJ%
   call mvn install -Djavacpp.copyResources -Dmaven.test.skip=true -Dmaven.javadoc.skip=true -Djavacpp.platform=windows-%MSYS2_ARCH% -pl %PROJ%
)
