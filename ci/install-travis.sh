...
if [[ "$OS" == "linux-x86" ]] || [[ "$OS" == "linux-x86_64" ]] || [[ "$OS" =~ android ]]; then
  ...
  docker exec -ti $DOCKER_CONTAINER_ID /bin/bash -xec "yum -q -y --disablerepo=cuda install rh-java-common-ant $SCL_ENABLE ccache clang gcc-c++ gcc-gfortran java-1.8.0-openjdk-devel ant python python36-devel python36-setuptools numpy swig git file which wget unzip tar bzip2 gzip xz patch make cmake3 autoconf-archive libtool perl nasm yasm alsa-lib-devel freeglut-devel gtk2-devel libusb-devel libusb1-devel zlib-devel SDL-devel libva-devel libxkbcommon-devel"
  ...
  if [ "$PROJ" == "tensorrt" ]; then
        python $TRAVIS_BUILD_DIR/ci/gDownload.py 1Q7ljbnIO1xz1hhCveOt3i4hln1K458Dk $HOME/downloads/tensorrt.tar.gz
...
if [ "$TRAVIS_OS_NAME" == "osx" ]; then
      ...
      if [[ "$PROJ" =~ cuda ]] || [[ "$EXT" =~ gpu ]]; then
        ...
        tar xvf $HOME/cudnn-10.1-osx-x64-v7.5.0.56.tgz
