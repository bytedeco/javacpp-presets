...
if [[ "$PROJ" =~ spinnaker ]]; then
       ...
       if [ "$OS" == "windows-x86_64" ]; then
           if [[ $(find /c/Downloads/Spinnaker-1.27.0.48_*_v140_x64.msi -type f -size +1000000c 2>/dev/null) ]]; then
             echo "Found spinnaker in cache and size seems ok"
           else
             echo "Downloading spinnaker to cache as not found"
             /c/python27/python $APPVEYOR_BUILD_FOLDER/ci/gDownload.py 1xIqjBwl3Q9GnEPcs9JdZBJNmOSWD8gRE /c/Downloads/Spinnaker-1.27.0.48_Binaries_v140_x64.msi
             /c/python27/python $APPVEYOR_BUILD_FOLDER/ci/gDownload.py 1l74LjV8rANk_P_CvXZOMSzi7J02YVkfD /c/Downloads/Spinnaker-1.27.0.48_SourceCode_v140_x64.msi
           fi
           # we can get these msi files by starting the installation from the exe file
           cmd.exe //c 'msiexec /quiet /i C:\Downloads\Spinnaker-1.27.0.48_Binaries_v140_x64.msi ADDLOCAL=ALL'
           cmd.exe //c 'msiexec /quiet /i C:\Downloads\Spinnaker-1.27.0.48_SourceCode_v140_x64.msi ADDLOCAL=ALL'
       elif [ "$OS" == "windows-x86" ]; then
           if [[ $(find /c/Downloads/Spinnaker-1.27.0.48_*_v140_x86.msi -type f -size +1000000c 2>/dev/null) ]]; then
             echo "Found spinnaker32 in cache and size seems ok"
           else
             echo "Downloading spinnaker32 to cache as not found"
             /c/python27/python $APPVEYOR_BUILD_FOLDER/ci/gDownload.py 1kufOTKKEGXbaQemjRi8Zy2H98FkL-k6W /c/Downloads/Spinnaker-1.27.0.48_Binaries_v140_x86.msi
             /c/python27/python $APPVEYOR_BUILD_FOLDER/ci/gDownload.py 1FNweWcn-keLqvy56xqz3Ve_T3adz43rW /c/Downloads/Spinnaker-1.27.0.48_SourceCode_v140_x86.msi
           fi
           # we can get these msi files by starting the installation from the exe file
           cmd.exe //c 'msiexec /quiet /i C:\Downloads\Spinnaker-1.27.0.48_Binaries_v140_x86.msi ADDLOCAL=ALL'
           cmd.exe //c 'msiexec /quiet /i C:\Downloads\Spinnaker-1.27.0.48_SourceCode_v140_x86.msi ADDLOCAL=ALL'
       fi
...
if [ "$PROJ" == "cuda" ] || [ "$EXT" == "-gpu" ]; then
       ...
       ./cuda_10.2.89_441.22_windows.exe -s
       sleep 60
       unzip ./cudnn-10.2-windows7-x64-v7.6.5.32.zip
       mv ./cuda/bin/*.dll /c/Program\ Files/NVIDIA\ GPU\ Computing\ Toolkit/CUDA/v10.2/bin
       mv ./cuda/include/*.h /c/Program\ Files/NVIDIA\ GPU\ Computing\ Toolkit/CUDA/v10.2/include
