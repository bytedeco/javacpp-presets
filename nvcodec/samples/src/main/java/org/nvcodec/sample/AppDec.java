...
import org.nvcodec.sample.encoder.YuvConverter;
import org.nvcodec.sample.exceptions.InvalidArgument;
import org.nvcodec.sample.util.AppDecUtils;
...
import static org.nvcodec.sample.util.NvCodecUtil.checkInputFile;
    ...
    public static void decodeMediaFile(CUctx_st cuContext, String inFilePath, String outFilePath, boolean outPlanar, Rectangle cropRectangle, Dimension resizeDimension) {
        try (FileOutputStream outputStream = new FileOutputStream(outFilePath)) {
            try (MP4Demuxer demuxer = MP4Demuxer.createMP4Demuxer(NIOUtils.readableChannel(new File(inFilePath)))) {
                ...
                NvDecoder nvDecoder = new NvDecoder(cuContext, false, nvcuvid.cudaVideoCodec_enum.cudaVideoCodec_H264, false, false, cropRectangle, resizeDimension, 0, 0, 1000);
