// Targeted by JavaCPP version 1.5.14: DO NOT EDIT THIS FILE

package org.bytedeco.tensorflowlite;

import java.nio.*;
import org.bytedeco.javacpp.*;
import org.bytedeco.javacpp.annotation.*;

import static org.bytedeco.tensorflowlite.global.tensorflowlite.*;


/** Sets the preparation callback for the registration.
 * 
 *  The callback is called when the inputs of operator have been resized.
 *  Please refer {@code prepare} of {@code TfLiteRegistration} for the detail.
 * 
 *  Deprecated: Use {@code TfLiteOperatorSetPrepareWithData} */
@Properties(inherit = org.bytedeco.tensorflowlite.presets.tensorflowlite.class)
public class Prepare_TfLiteOpaqueContext_TfLiteOpaqueNode extends FunctionPointer {
    static { Loader.load(); }
    /** Pointer cast constructor. Invokes {@link Pointer#Pointer(Pointer)}. */
    public    Prepare_TfLiteOpaqueContext_TfLiteOpaqueNode(Pointer p) { super(p); }
    protected Prepare_TfLiteOpaqueContext_TfLiteOpaqueNode() { allocate(); }
    private native void allocate();
    public native @Cast("TfLiteStatus") int call(TfLiteOpaqueContext context,
                            TfLiteOpaqueNode node);
}
