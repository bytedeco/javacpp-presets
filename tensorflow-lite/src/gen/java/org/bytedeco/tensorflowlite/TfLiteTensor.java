...
public class TfLiteTensor extends Pointer {
   ...
   *  that the buffer should have. NOTE: the product of elements of {@code dims}
   *  and the element datatype size should be equal to {@code bytes} below. */
  ...
  /** The delegate which knows how to handle {@code buffer_handle}.
   * 
  ...
  /** An integer buffer handle that can be handled by {@code delegate}.
   ...
   *  {@code delegate->CopyFromBufferHandle} can be called to copy the data from
   *  delegate buffer.
   * 
   *  WARNING: This is an experimental interface that is subject to change. */
   ...
   *  an input or output tensor). (e.g.  {@code dims} contains [1, 1, 1, 3] and
   *  {@code dims_signature} contains [1, -1, -1, 3]).  If no unknown dimensions exist
   *  then {@code dims_signature} is either null, or set to an empty array.  Use
   *  {@code TfLiteTensorGetDimsSignature} to get {@code dims_signature} if non-empty or
   *  otherwise fallback to {@code dims}.  Note that this field only exists when
