 ...
 * MKL-DNN 1.0.2  https://01.org/mkl-dnn
...
<project>
    ...
    <dependencies>
        <dependency>
            ...
            <version>1.0.2-1.5.2-SNAPSHOT</version>
        </dependency>
    </dependencies>
    <build>
...
//[Configure post-ops]
        ...
        try {
            ...
            if (e.getMessage().contains("status = " + mkldnn_unimplemented)) {
                System.err.println("Intel MKL-DNN does not have int8 convolution "
...
//[Create convolution primitive]
        ...
        conv.execute(s, new IntMemoryMap()
                .put(MKLDNN_ARG_SRC, conv_src_memory)
                .put(MKLDNN_ARG_WEIGHTS, conv_weights_memory)
                .put(MKLDNN_ARG_BIAS, conv_bias_memory)
                .put(MKLDNN_ARG_DST, conv_dst_memory));
//[Create convolution primitive]
