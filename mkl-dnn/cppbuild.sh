...
MKLDNN_VERSION=0.17-rc
MKLML_VERSION=2019.0.1.20180928
...
case $PLATFORM in
    linux-x86_64)
        ...
        "$CMAKE" -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=$INSTALL_PATH -DCMAKE_CXX_FLAGS='-Wl,-rpath,$ORIGIN/' -DARCH_OPT_FLAGS='' -DWITH_EXAMPLE=OFF -DWITH_TEST=OFF -Wno-error=unused-result
        make -j $MAKEJ
        make install/strip
    ...
    macosx-x86_64)
        ...
        "$CMAKE" -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=$INSTALL_PATH -DARCH_OPT_FLAGS='' -DWITH_EXAMPLE=OFF -DWITH_TEST=OFF
        make -j $MAKEJ
        make install/strip
    ...
    windows-x86_64)
        ...
        "$CMAKE" -G "MSYS Makefiles" -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=$INSTALL_PATH -DARCH_OPT_FLAGS='' -DWITH_EXAMPLE=OFF -DWITH_TEST=OFF
        make -j $MAKEJ
        make install/strip
