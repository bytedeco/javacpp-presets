...
if [[ -f "$CPYTHON_PATH/include/python3.14/Python.h" ]]; then
    ...
    export PATH="$CPYTHON_PATH/lib/python3.14/bin/:$PATH"
...
if ! $PYTHON_BIN_PATH -m pip install --target=$PYTHON_LIB_PATH $TOOLS; then
    ...
    "$CPYTHON_HOST_PATH/bin/python3.14" -m pip install --target="$CPYTHON_HOST_PATH/lib/python3.14/" crossenv==1.4 $TOOLS
    "$CPYTHON_HOST_PATH/bin/python3.14" -m crossenv "$PYTHON_BIN_PATH" crossenv
    source crossenv/bin/activate
    cross-expose cython
    chmod +x $CPYTHON_HOST_PATH/lib/python3.14/bin/*
    export PATH="$CPYTHON_HOST_PATH/lib/python3.14/bin/:$PATH"
    export PYTHON_BIN_PATH="python"
fi

case $PLATFORM in
    linux-armhf)
        ATLAS=None CC="arm-linux-gnueabihf-gcc -std=c99" "$PYTHON_BIN_PATH" -m pip install . --prefix $INSTALL_PATH --config-settings=build-dir=build
    ...
    linux-arm64)
        ...
        if [[ $HOST_ARCH == "aarch64" ]]; then
          ...
          ATLAS=None CC="gcc -std=c99" "$PYTHON_BIN_PATH" -m pip install . --prefix $INSTALL_PATH --config-settings=build-dir=build
          strip $(find ../ -iname *.so)
        else
          ...
          ATLAS=None CC="aarch64-linux-gnu-gcc -mabi=lp64" CFLAGS="-O2" "$PYTHON_BIN_PATH" -m pip install . --prefix $INSTALL_PATH --config-settings=build-dir=build
          aarch64-linux-gnu-strip $(find ../ -iname *.so)
        fi
        ;;
    linux-ppc64le)
        ATLAS=None CC="powerpc64le-linux-gnu-gcc -m64" "$PYTHON_BIN_PATH" -m pip install . --prefix $INSTALL_PATH --config-settings=build-dir=build
    ...
    linux-x86)
        ...
        ;;
    linux-x86_64)
        ATLAS=None CC="gcc -m64" "$PYTHON_BIN_PATH" -m pip install . --prefix $INSTALL_PATH --config-settings=build-dir=build
        strip $(find ../ -iname *.so)
        ;;
    macosx-*)
