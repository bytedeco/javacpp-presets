...
public static final int
    ...
    /** Support for PanFS */
    CU_FILE_PANFS_SUPPORTED = 14,
    /**Maximum FS supported */
    CU_FILE_MAX_TARGET_TYPES = 15;
...
public static final int
        ...
        /** Vanilla posix io mode. properties.vanilla_posix_io_mode*/
        CU_FILE_VANILLA_POSIX_IO_MODE = 2,
        /** alias for backward compatibility */
        CU_FILE_POSIX_IO_MODE = CU_FILE_VANILLA_POSIX_IO_MODE,
        /** Fallback io mode. properties.gds_fallback_io*/
...
/**
 * \brief RDMA-specific error codes
 *
 * These error codes are used between cuObject and cuFile layers to signal QP reset conditions.
 * Base: 6000 (separate from CUFILEOP_BASE_ERR = 5000)
 */
public static final int CU_FILE_RDMA_ERR_BASE =        6000;
/** RDMA operation succeeded */
public static final int CU_FILE_RDMA_SUCCESS =         0;
/** QP reset requested (triggers reset action) */
public static final int CU_FILE_RDMA_RESET_TOKEN =     (CU_FILE_RDMA_ERR_BASE + 1);
/** QP reset failed */
public static final int CU_FILE_RDMA_RESET_TOKEN_FAILED =    (CU_FILE_RDMA_ERR_BASE + 2);
 ...
 * If the driver is not opened, it will return the staged/default properties
 * If the driver is opened, it will return the current properties
 *
 * @return CU_FILE_SUCCESS on success
 * @return CU_FILE_INVALID_VALUE if props is NULL
 *
 ...
 * This takes place before the driver is opened. No-op if driver is already open.
 *
 ...
 *
 * @return CU_FILE_SUCCESS on success
 * @return CU_FILE_INVALID_VALUE if invalid value is passed
 *
 ...
 * @return CU_FILE_SUCCESS on success
 * @return CU_FILE_INVALID_VALUE if invalid value is passed
 * @return CU_FILE_CUDA_DRIVER_ERROR if CUDA driver APIs fail
 * @return CU_FILE_DRIVER_UNSUPPORTED_LIMIT if unsupported limit is passed
 *
...
// Targeting ../cufile/CUfileIOVec_t.java



/**
 * \brief Reads data from an offset in a registered file handle to one or more buffers in a specified device or host memory
 *
 * @param fh \ref CUfileHandle_t opaque file handle
 * @param iov  array of IO vectors for buffers in device or host memory
 * @param iovcnt  number of IO vectors in iov array
 * @param file_offset  file-offset from begining of the file
 ...
 * @return  all other errors will return a negative integer value of \ref CUfileOpError enum value.
 *
 * \note  If the buffers in the iov array are not registered with \ref cuFileBufRegister, the data will be buffered through preallocated pinned buffers if needed.
 * \note  This is useful for applications that need to perform IO to unaligned file offsets and/or size. This is also recommended for cases where the BAR1 memory size is smaller than the size of the allocated memory.
 *
 * @see cuFileBufRegister
 * @see cuFileHandleRegister
 * @see cuFileRead
...
public static native @Cast("ssize_t") long cuFileReadv(CUfileHandle_t fh, @Const CUfileIOVec_t iov, @Cast("size_t") long iovcnt, @Cast("off_t") long file_offset, @Cast("unsigned") int flags);

/**
 * \brief Writes data to an offset in a registered file handle from one or more buffers in a specified device or host memory
 *
 * @param fh \ref CUfileHandle_t opaque file handle
 * @param iov  array of IO vectors for buffers in device or host memory
 * @param iovcnt  number of IO vectors in iov array
...
public static native @Cast("ssize_t") long cuFileWritev(CUfileHandle_t fh, @Const CUfileIOVec_t iov, @Cast("size_t") long iovcnt, @Cast("off_t") long file_offset, @Cast("unsigned") int flags);
...
/**
 *\brief
 * Exports a copy of the detected PCIe topology to a specified file in JSON format.
 * 
 * @param filename the path and file name where the exported PCIe topology should be written.
 * 
 * \note  This is useful for applications that need to validate the detected PCIe topology.
 *
 * @return CU_FILE_SUCCESS on success
 * @return CU_FILE_INVALID_VALUE if the filename is invalid.
 * @return CU_FILE_INTERNAL_ERROR if the PCIe topology is not available.
 * @return CU_FILE_DRIVER_* if there are errors opening/closing the driver.
 *
 */
public static native @ByVal CUfileError_t cuFileExportPCIeTopology(@Cast("const char*") BytePointer filename);
public static native @ByVal CUfileError_t cuFileExportPCIeTopology(String filename);
...
public static final int
	...
	CUFILE_PARAM_PROPERTIES_POSIX_IO_MODE = 12,
	CUFILE_PARAM_GDS_FALLBACK_IO = 13;
 ...
 * @param barSize  : Pointer to store the BAR size in KiB
 *
...
/**
 * \brief Set GPU bounce buffer slab size and count parameters as a pair
 *
 * This allows programmatic configuration of GPU bounce buffer slab pools
 * before the driver is opened. Must be called before cuFileDriverOpen().
 *
 * @param size_values Array of slab sizes in KB (must be 4KB aligned, ascending order)
 * @param count_values Array of slab counts (number of buffers per slab size)
 * @param len Number of elements in both arrays (must match)
 *
 * @return CU_FILE_SUCCESS on success
 * @return CU_FILE_INVALID_VALUE if inputs are invalid
 * @return CU_FILE_DRIVER_ALREADY_OPEN if driver is already open
 */
public static native @ByVal CUfileError_t cuFileSetParameterGpuBounceBufferSlabArray(
                                     @Cast("const size_t*") SizeTPointer size_values,
                                     @Cast("const size_t*") SizeTPointer count_values,
                                     int len);

/**
 * \brief Get GPU bounce buffer slab size and count parameters as a pair
 *
 * Retrieves the configured or staged GPU bounce buffer slab configuration.
 * Can be called before or after cuFileDriverOpen().
 *
 * @param size_values Buffer to receive slab sizes in KB
 * @param count_values Buffer to receive slab counts
 * @param len Buffer size (must match the actual parameter length)
 ...
 * @return CU_FILE_INVALID_VALUE if inputs are invalid or buffer size doesn't match
 */
public static native @ByVal CUfileError_t cuFileGetParameterGpuBounceBufferSlabArray(
                                     @Cast("size_t*") SizeTPointer size_values,
                                     @Cast("size_t*") SizeTPointer count_values,
                                     int len);
