...
case $PLATFORM in
    ...
    linux-ppc64le)
        ...
        if [[ "$MACHINE_TYPE" =~ ppc64 ]]; then
          ...
          CC="powerpc64le-linux-gnu-gcc" CXX="powerpc64le-linux-gnu-g++" PKG_CONFIG_PATH=../lib/pkgconfig/ ./configure --prefix=.. $DISABLE "${ENABLE//--enable-libfreetype}" --enable-libxcb --cc="powerpc64le-linux-gnu-gcc -m64" --extra-cflags="-I../include/" --extra-ldflags="-L../lib/" --enable-cross-compile --target-os=linux --arch=ppc64le-linux --extra-libs="-lstdc++ -ldl"  
        fi
        make -j $MAKEJ
        make install
        ;;
