...
@Properties(
    ...
    value = {
        @Platform(cinclude = {"<libavfilter/avfilter.h>", "<libavfilter/buffersink.h>", "<libavfilter/buffersrc.h>", "<libavfilter/version_major.h>", "<libavfilter/version.h>"}, link = "avfilter@.11"),
        @Platform(value = "windows", preload = "avfilter-11")
    }
)
public class avfilter implements InfoMapper {
    public void map(InfoMap infoMap) {
        infoMap.put(new Info("AVFilterPool", "AVFilterCommand", "AVFilterChannelLayouts", "FFFrameQueue").cast().pointerTypes("Pointer"))
               ...
               .put(new Info("FF_API_SWS_PARAM_OPTION", "FF_API_BUFFERSINK_ALLOC", "FF_API_PAD_COUNT", "FF_API_LIBPLACEBO_OPTS",
                             "FF_API_LINK_PUBLIC", "FF_API_BUFFERSINK_OPTS", "FF_API_CONTEXT_PUBLIC", "FF_API_LIBNPP_SUPPORT").define().translate().cppTypes("bool"))
